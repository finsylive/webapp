...
const INTEREST_OPTIONS = [
  { id: 'exploring', label: 'Discover & Connect', description: 'Explore startups, learn from builders', color: 'emerald' },
  { id: 'building', label: 'Build & Launch', description: 'Ship your startup, find resources', color: 'amber' },
  { id: 'investing', label: 'Invest & Fund', description: 'Find deals, manage deal flow', color: 'violet' },
] as const;
...
const SettingsPage = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('users').select('primary_interest').eq('id', user.id).single()
      .then(({ data }: { data: { primary_interest: string | null } | null }) => {
        if (data?.primary_interest) setPrimaryInterest(data.primary_interest);
      });
  }, [user]);
    ...
    <div className="space-y-6 px-4">
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          />
          <SettingCard
            icon={<IconBox icon={Compass} />}
            title="Your Interests"
            description="What brings you to Ments"
  ...
  const handleInterestChange = async (interest: string) => {
    ...
    try {
      const res = await fetch('/api/settings/interests', {
        ...
        body: JSON.stringify({ primary_interest: interest }),
      });
      if (!res.ok) {
        setPrimaryInterest(primaryInterest); // revert
      }
    } catch {
      setPrimaryInterest(primaryInterest); // revert
    } finally {
  ...
  const InterestsSettings = () => (
    <div className="space-y-6">
      <BackButton onClick={() => setActiveSection('main')} title="Your Interests" />
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        <div className="flex items-center">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Primary interest</h2>
            <p className="text-sm text-muted-foreground">This shapes your feed and recommendations</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <div className="space-y-3">
          {INTEREST_OPTIONS.map((option) => {
            const isActive = primaryInterest === option.id;
            return (
              <button
                key={option.id}
